...
async def wave_get_product_images(PNumber):
    """Získává obrázky produktů z wave-distribution.de"""
    import requests
    from bs4 import BeautifulSoup
    ...
    # 1. Vyhledávací stránka
    search_url = f"https://www.wave-distribution.de/listing.xhtml?q={PNumber}"
    print(f"[DEBUG] Fetching search page: {search_url}")

    try:
        # Získání vyhledávacích výsledků
        response = requests.get(search_url, headers=headers, timeout=10)
        if response.status_code != 200:
            print(f"[ERROR] Failed to fetch search page: {response.status_code}")
            return []

        soup = BeautifulSoup(response.text, "html.parser")

        # 2. Nalezení odkazu na produkt
        product_link = soup.find('a', class_="card align-content-center productBox boxCounter campaign-timer-container",
                                 href=True)
        if not product_link:
            print("[ERROR] Product link not found")
            return []

        # 3. Sestavení URL produktu
        product_url = product_link['href']
        if product_url.startswith("/"):
            product_url = "https://www.wave-distribution.de" + product_url
        print(f"[DEBUG] Found product URL: {product_url}")

        # 4. Načtení stránky produktu
        product_response = requests.get(product_url, headers=headers, timeout=10)
        if product_response.status_code != 200:
            print(f"[ERROR] Failed to fetch product page: {product_response.status_code}")
            return []

        product_soup = BeautifulSoup(product_response.text, "html.parser")

        # 5. Nalezení všech obrázků - rozšířené vyhledávání
        image_urls = []

        # Hledáme všechny img tagy uvnitř buttonů
        buttons = product_soup.find_all('button', class_='btn btn-link p-0 w-100')
        for button in buttons:
            img = button.find('img')
            if img:
                src = img.get('src') or img.get('srcset')
                if src:
                    # Získání vyšší kvality obrázku ze srcset pokud je dostupná
                    if 'srcset' in img.attrs:
                        srcset = img['srcset']
                        # Vybereme největší verzi obrázku (600x600)
                        for source in srcset.split(','):
                            if '600x600' in source:
                                src = source.strip().split(' ')[0]
                                break
                    ...
                    if src.startswith("//"):
                        ...
                        src = "https://www.wave-distribution.de" + src

                    image_urls.append(src)

        # Alternativní metoda pro případ, že předchozí selže
        if not image_urls:
            print("[DEBUG] Trying alternative image search method")
            img_tags = product_soup.find_all('img', class_='productPicture')
            for img in img_tags:
                src = img.get('src') or img.get('srcset')
                if src:
                    if src.startswith("//"):
        ...
        print(f"[ERROR] wave_get_product_images: {e}")
        return []
